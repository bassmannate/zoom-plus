...
export function getFunctionName(stackLevel = 0) {
    let error = new Error();
    if (!error.stack)
        return "unknown";
...
export function getExceptionErrorString(err, extra) {
...
export function bytesWithCharactersToString(bytes) {
 ...
 */
export function hexStringToUint8Array(str, separator = '') {
...
export function partialArrayMatch(bigArray, smallArray, bigArrayOffset = 0) {
...
export function partialArrayStringMatch(bigArray, str, bigArrayOffset = 0) {
    return bigArray.length - bigArrayOffset >= str.length && bigArray.slice(bigArrayOffset, bigArrayOffset + str.length).every((element, index) => element === (str.charCodeAt(index) & 0xFF));
}
/**
 *
 ...
 */
export function getNumberFromBits(data, startBit, endBit) {
 ...
 */
export function setBitsFromNumber(data, startBit, endBit, value) {
    let startByte = Math.floor(startBit / 8);
    let endByte = Math.floor(endBit / 8);
    let startBitOffset = startBit % 8;
    let endBitOffset = endBit % 8;
 ...
 */
export function crc32(data, offset = 0, end = -1) {
    if (end === -1)
        end = data.length - 1;
    if (crc32table === null)
 ...
 */
export function seven2eight(sevenBitBytes, start = 0, end = -1) {
    if (end === -1)
    ...
    let eightBitBytes = new Uint8Array(Math.floor((end - start + 1) / 8) * 7 + (remainder < 2 ? 0 : remainder - 1));
 ...
 */
export function getNumberOfEightBitBytes(numberOf7BitBytes) {
 ...
 */
export function eight2seven(eightBitBytes, start = 0, end = -1) {
    if (end === -1)
        end = eightBitBytes.length - 1;
    let sevenBitBytes = new Uint8Array((end - start + 1) + Math.ceil((end - start + 1) / 7));
...
export function compareBuffers(newBuffer, oldBuffer, doLogging = false) {
...
export function sleepForAWhile(timeoutMilliseconds) {
