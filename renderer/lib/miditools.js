...
let getMIDIDeviceListIsRunning = false;
/**
 ...
 * @returns
 */
export async function getMIDIDeviceList(midi, inputs, outputs, timeoutMilliseconds = 100, logging = false) {
    ...
    return new Promise(async (resolve) => {
        let currentOutput;
        let timeoutId;
        let listeners = new Map(); // used for removing listeners after we're done
        let openedInputs = new Array();
        let openedOutputs = new Array();
        let midiDevices = [];
        ...
        for (let [id, input] of inputs) {
            ...
            let handleSysex = (deviceHandle, data) => {
                if (currentOutput !== undefined && data.length >= 15 && data[0] == 0xF0 && data[1] == 0x7E && data[3] == 0x06 && data[4] == 0x02 &&
                    ...
                    let dataOffset = data[5] != 0 ? 0 : 2; // if manufacturer ID is 3 bytes instead of just 1, data after that is offset with 2 bytes.
                    let inputID = input.id;
                    let inputName = input.name;
                    let outputID = currentOutput.id;
                    let outputName = currentOutput.name;
                    let isInput = true;
                    let isOutput = true;
                    let manufacturerID = data[5] != 0 ? [data[5]] : [data[5], data[6], data[7]];
                    ...
                    let familyCode = [data[6 + dataOffset], data[7 + dataOffset]];
                    let modelNumber = [data[8 + dataOffset], data[9 + dataOffset]];
                    let deviceName = getDeviceName(manufacturerID, familyCode, modelNumber);
                    let versionNumber = [data[10 + dataOffset], data[11 + dataOffset], data[12 + dataOffset], data[13 + dataOffset]];
                    ...
                    if (logging)
                        shouldLog(LogLevel.Info) && console.log(`      Received sysex ID reply ${bytesToHexString(data, " ")} -> ${JSON.stringify(description)}`);
                    midiDevices.push(description);
                    if (logging)
                        shouldLog(LogLevel.Info) && console.log(`  Clearing timeout (${timeoutId})`);
        ...
        async function sendAndSetTimeout() {
            ...
            if (currentOutput === undefined)
                ...
                let localTimeoutId = setTimeout(() => {
                    if (logging)
                        shouldLog(LogLevel.Info) && console.log(`      Timed out (${localTimeoutId}) for device ${currentOutputID} "${currentOutputName}"`);
                    sendAndSetTimeout();
                }, timeoutMilliseconds);
                if (logging)
                    shouldLog(LogLevel.Info) && console.log(`  Set timeout (${localTimeoutId})`);
        ...
        async function done() {
            ...
            let possibleDuplicates = new Map(); // maps from inputID to list of midiDevices indexes
            ...
            let duplicates = new Map([...possibleDuplicates].filter(([key, indexes]) => indexes.length > 1));
            let indexesToDelete = []; // list of indexes for items to delete from midiDevices
            ...
            let unpairedInputs = [...inputs.values()].filter((input) => midiDevices.find((device) => device.inputName == input.name) == undefined // include only inputs that are not already in the midiDevices
            );
            let unpairedOutputs = [...outputs.values()].filter((output) => midiDevices.find((device) => device.outputName == output.name) == undefined // include only outputs that are not already in the midiDevices
            );
            ...
            for (let inputIndex = 0; inputIndex < unpairedInputs.length; inputIndex++) {
                ...
                if (outputIndex != -1) { // We found an input and an output with the same name
            ...
            for (let similarity = 1; similarity <= 4; similarity++) {
                for (let inputIndex = 0; inputIndex < unpairedInputs.length; inputIndex++) {
                    ...
                    if (outputIndex != -1) { // We found an input and an output with a similar name
...
function manufacturerIDsAreEqual(id1, id2) {
 ...
 */
function getDeviceName(manufacturerID, familyCode, modelNumber) {
 ...
 */
export function isMIDIIdentityResponse(data) {
 ...
 */
export function isSysexString(data) {
    return data.length >= 3 && data[0] == 0xF0 && data[data.length - 1] == 0xF7;
}
export function getChannelMessage(data) {
    if (data.length < 1)
        ...
        return [data[0] & 0b11110000, data[0] & 0b00001111, data.length > 1 ? data[1] : -1, data.length > 2 ? data[2] : -1];
}
/**
 * Map from manufacturerId, familyCode, and modelNumber to device name
 */
let MIDIDeviceHexStringToName = {
...
//# sourceMappingURL=miditools.js.map