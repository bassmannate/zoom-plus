...
export class EffectSettings {
    ...
    constructor(numParameters = 0) {
        this.parameters = new Array(numParameters);
        if (numParameters > 0)
...
export class ZoomPatch {
    ...
    tempo = 0;
    currentEffectSlot = 0;
    static NAME_LENGTH_MS_PLUS = 32;
    static NAME_LENGTH_G2_G2X_FOUR = 16;
   ...
   */
    ...
    static createPreampSlotBits(effectSettings) {
        let preampBits = 0;
    ...
    static createLineSelSlotBits(effectSettings) {
        let lineSelBits = 0;
        for (let i = 0; i < effectSettings.length; i++) {
    ...
    static swapBits(bitPattern, bit1, bit2) {
    ...
    static deleteBitAndShiftUpperBits(bitPattern, bit) {
        let upperBits = (bitPattern >> (bit + 1)) << bit;
        let lowerBits = bitPattern & (0b11111111 >> (8 - bit));
        return lowerBits + upperBits;
    }
    static insertBitAndShiftUpperBits(bitPattern, bit, value) {
    ...
    static prm2BitPattern = 0b101010000110010000100000;
    /**
     *
     ...
     */
    static effectSlotToPrm2BitPattern(selectedEffectSlot, totalNumberOfSlots) {
    ...
    PTCF = null;
    length = null; // NB! This length includes the 4 byte ID and the 4 byte length value, in other words it is the total patch length starting from the P in PTCF. This is different from the chunk lengths below, which does not include these 8 bytes.
    version = null;
    numEffects = null;
    maxNumEffects = 0;
    target = null;
    ptcfUnknown = null; // 6 bytes
    ptcfShortName = null; // For patches delivered with MS+ pedals, this is always the 10 first characters of nameName
    ids = null;
    idBuffer = new ArrayBuffer(32 * 16); // storage buffer for ids. ids is a view into idBuffer.
    ptcfChunk = null; // Raw unparsed PTCF chunk, including the "PTCF" ID and 4 bytes for the length value
    chunks = new Map(); // Raw unparsed chunks
    // Unknown
    TXJ1 = null; // 4 + 4 + txj1Length bytes
    txj1Length = null; // 4 bytes, chunk length does not include the length of the ID (4 bytes) and length bytes (4 bytes) itself 
    txj1DescriptionJapanese = null; // txj1Length bytes
    // Description
    TXE1 = null; // 4 + 4 + txe1Length bytes
    txe1Length = null; // 4 bytes
    txe1DescriptionEnglish = null; // txe1Length bytes
    EDTB = null; // 4 + 4 + numEffects * 24 bytes == 4 + 4 + edtbLength bytes (numEffects * PTCF_EDTB_REVERSED_BYTES_SIZE)
    edtbLength = null; // 4 bytes
    edtbReversedBytes = null; // numEffects * 24 bytes == edtbLength bytes (PTCF_EDTB_REVERSED_BYTES_SIZE)
    edtbEffectSettings = null;
    PRM2 = null; // 4 + 4 + prm2Length
    prm2Length = null; // 4 bytes
    prm2InvalidEffectSlot = null; // based on byte 2 and 3 in prm2Unknown, bitfield for slot(s) with unknown effect
    prm2Byte2Lower6Bits = null; // only seen 0 for these bits
    prm2Byte3Upper4Bits = null; // only seen 0 for these bits
    ...
    prm2Byte9Lower5Bits = null; // only seen 0 for these bits
    prm2Byte10Bit5 = null; // only seen 0 for this bit
    prm2PatchVolume = null; // based on byte 9 and 10 in prm2Unknown
    prm2EditEffectSlot = null; // based on byte 10 in prm2Unknown
    prm2EditEffectSlotBits = null; // based on byte 11 and 12 in prm2Unknown
    prm2Byte13 = null; // byte 13 in prm2Unknown - MS-50G+, 0 or 0x8C ?. Right-position?. See https://github.com/mungewell/zoom-zt2/issues/101
    prm2Byte14 = null; // byte 14 in prm2Unknown - MS-50G+, 0 or 0x08 ?
    prm2PreampSlot = null; // based on byte 20 in prm2Unknown
    prm2Byte20Bit1And8 = null; // only seen 0 for these bits
    prm2BPMSlot = null; // based on byte 21 and 22 in prm2Unknown, bitfield for slot with BPM effect
    prm2Byte21Lower4Bits = null; // only seen 0 for these bits 
    prm2Byte22Bits3To7 = null; // only seen 0 for these bits
    prm2LineSelSlot = null; // based on byte 22 and 23 in prm2Unknown, bitfield for slot with Line Selector
    prm2Byte23Upper3Bits = null; // byte 23 in prm2Unknown - Line Selector on MS-70CDR+
    prm2Byte24 = null; // byte 24 in prm2Unknown - MS-50G+, 0 or 0x80
    prm2Byte25 = null; // byte 25 in prm2Unknown
    prm2Byte26 = null; // byte 26 in prm2Unknown
    prm2Byte27 = null; // byte 27 in prm2Unknown
    prm2Byte28 = null; // byte 28 in prm2Unknown
    prm2Routing = null; // based the first 4 bits of the last byte in prm2Unknown
    prm2Tempo = null; // based on the two last bytes in prm2Unknown
    prm2Buffer = null; // prm2Length bytes
    NAME = null;
    nameLength = null; // 4 bytes
    nameName = null; // 28 bytes == nameLength bytes. The last four characters are sometimes (always?) 0x00.
    // Zoom MS OriGinal pedal info below
    MSOG = null; // This is not a named chunk. The original MS pedals doesn't have a chunk-based layout but rather a fixed structure.
    msogEffectSettings = null;
    msogEffectsReversedBytes = null; // numEffects * 18 bytes (MSOG_REVERSED_BYTES_SIZE)
    msogUnknown1 = null; // 3 bytes
    msogName = null; // 10 bytes
    msogUnknown2 = null; // 1 byte
    msogTempo = null; // 8 bits based on the last 2 bytes in msogUnknown1
    msogEditEffectSlot = null; // 3 bits based on the two first bytes in msogUnknown1
    msogDSPFullBits = null; // 6 bits based on the first byte in mspgUnknown1
    msogNumEffects = null; // 3 bits based on the second byte in mspgUnknown1
    // length: null | number = null;
    // ids: null | Uint32Array = null;
    msogDataBuffer = null; // Complete 8-bit data buffer for the patch
    clone() {
        ...
        for (let i = 0; i < properties.length; i++) {
            ...
            if (property === null)
                ...
                let clonedMap = patch[propertyName];
                property.forEach((chunkBytes, chunkName) => clonedMap.set(chunkName, new Uint8Array(chunkBytes)));
            }
            else if (property.constructor.name === "Array") {
                if (property[0].constructor.name === "Uint8Array")
                    patch[propertyName] = Array.from(property, (e, i) => new Uint8Array(e));
                else if (property[0].constructor.name === "EffectSettings")
                    patch[propertyName] = Array.from(property, (e, i) => e.clone());
            }
            else if (property.constructor.name === "Uint32Array")
                patch[propertyName] = new Uint32Array(property);
            else if (property.constructor.name === "Uint8Array")
                patch[propertyName] = new Uint8Array(property);
            else
    ...
    writeString(data, offset, str, enforceBufferLength = -1) {
    ...
    readInt32Array(patch, offset, length, array = null) {
    ...
    readPTCFChunks(patch, offset, remainingPatchLength) {
    ...
    readPTCF(data, offset) {
        ...
        this.version = this.readInt32(data, offset);
        offset += 4;
        this.numEffects = this.readInt32(data, offset);
        offset += 4;
        this.target = this.readInt32(data, offset);
        offset += 4;
        this.ptcfUnknown = data.slice(offset, offset + 6);
        offset += 6;
        this.ptcfShortName = this.readString(data, offset, 10);
        offset += 10;
        ...
        if (chunkData === undefined) {
            ...
            if (this.txj1Length != null && this.txj1Length > 0) {
                this.txj1DescriptionJapanese = chunkData.slice(chunkOffset, chunkOffset + this.txj1Length);
                chunkOffset += this.txj1Length;
            }
        }
        chunkID = "TXE1";
        chunkData = this.chunks.get(chunkID);
        if (chunkData === undefined) {
            ...
            if (this.txe1Length != null && this.txe1Length > 0) {
                this.txe1DescriptionEnglish = this.readString(chunkData, chunkOffset, this.txe1Length);
                chunkOffset += this.txe1Length;
            }
        }
        // EDTB chunk
        // Bit  # bits  Meanining
        ...
        if (chunkData === undefined) {
            ...
            if (this.numEffects === null) {
                ...
                this.edtbEffectSettings = new Array();
                for (let i = 0; i < this.numEffects; i++) {
                    this.edtbReversedBytes[i] = chunkData.slice(chunkOffset, chunkOffset + PTCF_EDTB_REVERSED_BYTES_SIZE).reverse();
                    chunkOffset += PTCF_EDTB_REVERSED_BYTES_SIZE;
                    let bitpos = this.edtbReversedBytes[i].length * 8 - 1;
                    let effectSettings = new EffectSettings();
                    effectSettings.enabled = (getNumberFromBits(this.edtbReversedBytes[i], bitpos, bitpos) === 1);
                    bitpos -= 1;
                    effectSettings.id = getNumberFromBits(this.edtbReversedBytes[i], bitpos - 28, bitpos);
                    bitpos -= 29;
                    effectSettings.parameters = new Array();
                    for (let p = 0; p < 5 && bitpos - 12 >= 0; p++) {
                        let parameter = getNumberFromBits(this.edtbReversedBytes[i], bitpos - 11, bitpos);
                        bitpos -= 12;
        ...
        if (chunkData === undefined) {
            ...
            if (this.prm2Length != null && this.prm2Length > 2) {
                this.prm2Buffer = chunkData.slice(chunkOffset, chunkOffset + this.prm2Length);
                chunkOffset += this.prm2Length;
        ...
        if (chunkData === undefined) {
            ...
            if (this.nameLength != null && this.nameLength > 0) {
                this.nameName = this.readString(chunkData, chunkOffset, this.nameLength);
                chunkOffset += this.nameLength;
    ...
    verifyPrm2Buffer() {
        ...
        this.setPrm2BufferFromDerivedValues(compareBuffer, true, this.prm2InvalidEffectSlot, this.prm2PatchVolume, this.prm2EditEffectSlot, this.edtbEffectSettings.length, this.prm2PreampSlot, this.prm2BPMSlot, this.prm2LineSelSlot, this.prm2Tempo, this.prm2Routing, this.prm2Byte2Lower6Bits, this.prm2Byte3Upper4Bits, this.prm2Byte9Lower5Bits, this.prm2Byte10Bit5, this.prm2Byte13, this.prm2Byte14, this.prm2Byte20Bit1And8, this.prm2Byte21Lower4Bits, this.prm2Byte22Bits3To7, this.prm2Byte23Upper3Bits, this.prm2Byte24, this.prm2Byte25, this.prm2Byte26, this.prm2Byte27, this.prm2Byte28);
        return compareBuffers(compareBuffer, this.prm2Buffer, true);
    }
    setPrm2BufferFromDerivedValues(prm2Buffer, clear, prm2InvalidEffectSlot, prm2PatchVolume, prm2EditEffectSlot, totalNumberOfSlots, prm2PreampSlot, prm2BPMSlot, prm2LineSelSlot, prm2Tempo, prm2Routing, prm2Byte2Lower6Bits = 0, prm2Byte3Upper4Bits = 0, prm2Byte9Lower5Bits = 0, prm2Byte10Bit5 = 0, prm2Byte13 = 0, prm2Byte14 = 0, prm2Byte20Bit1And8 = 0, prm2Byte21Lower4Bits = 0, prm2Byte22Bits3To7 = 0, prm2Byte23Upper3Bits = 0, prm2Byte24 = 0, prm2Byte25 = 0, prm2Byte26 = 0, prm2Byte27 = 0, prm2Byte28 = 0) {
    ...
    buildPTCFChunk(nameLength = 0) {
        ...
        success &&= (result !== 0);
        offset = result = this.writeInt32(ptcfChunk, offset, ptcfTotalLength);
        success &&= (result !== 0);
        offset = result = this.writeInt32(ptcfChunk, offset, this.version);
        success &&= (result !== 0);
        offset = result = this.writeInt32(ptcfChunk, offset, this.numEffects);
        success &&= (result !== 0);
        offset = result = this.writeInt32(ptcfChunk, offset, this.target);
        success &&= (result !== 0);
        offset = result = this.writeSlice(ptcfChunk, offset, this.ptcfUnknown);
        success &&= (result !== 0);
        offset = result = this.writeString(ptcfChunk, offset, this.ptcfShortName);
        success &&= (result !== 0);
        ...
        if (this.TXJ1 === null) {
            ...
            success &&= (result !== 0);
            offset = result = this.writeInt32(ptcfChunk, offset, this.txj1Length);
            success &&= (result !== 0);
        ...
        if (this.TXE1 === null) {
            ...
            success &&= (result !== 0);
            offset = result = this.writeInt32(ptcfChunk, offset, this.txe1Length);
            success &&= (result !== 0);
        ...
        success &&= (result !== 0);
        offset = result = this.writeInt32(ptcfChunk, offset, this.edtbLength);
        success &&= (result !== 0);
        for (let i = 0; i < this.numEffects; i++) {
            let reversedBytes = new Uint8Array(PTCF_EDTB_REVERSED_BYTES_SIZE);
            ...
            setBitsFromNumber(reversedBytes, bitpos, bitpos, effectSettings.enabled ? 1 : 0);
            bitpos -= 1;
            setBitsFromNumber(reversedBytes, bitpos - 28, bitpos, effectSettings.id);
            bitpos -= 29;
            let parameterIndex = 0;
            for (let p = 0; p < 5 && bitpos - 12 >= 0; p++) {
                let parameter = effectSettings.parameters[parameterIndex++];
                setBitsFromNumber(reversedBytes, bitpos - 11, bitpos, parameter);
                bitpos -= 12;
            }
            for (let p = 5; p < 8 && bitpos - 8 >= 0; p++) {
                let parameter = effectSettings.parameters[parameterIndex++];
                setBitsFromNumber(reversedBytes, bitpos - 7, bitpos, parameter);
                bitpos -= 8;
            }
            for (let p = 8; p < 12 && bitpos - 12 >= 0; p++) {
                let parameter = effectSettings.parameters[parameterIndex++];
            ...
            offset = result = this.writeSlice(ptcfChunk, offset, rightOrderBytes);
            success &&= (result !== 0);
        }
        ...
        if (this.PRM2 === null) {
            ...
            offset = result = this.writeString(ptcfChunk, offset, this.PRM2);
            success &&= (result !== 0);
            offset = result = this.writeInt32(ptcfChunk, offset, this.prm2Length);
            success &&= (result !== 0);
            offset = result = this.writeSlice(ptcfChunk, offset, this.prm2Buffer);
            success &&= (result !== 0);
        }
        ...
        if (this.NAME === null) {
            ...
            success &&= (result !== 0);
            offset = result = this.writeInt32(ptcfChunk, offset, this.nameLength);
            success &&= (result !== 0);
            offset = result = this.writeString(ptcfChunk, offset, this.nameName, this.nameLength);
            success &&= (result !== 0);
        }
    ...
    buildMSDataBuffer() {
        ...
        for (let i = 0; i < this.maxNumEffects; i++) {
            ...
            setBitsFromNumber(reversedBytes, bitpos, bitpos, effectSettings.enabled ? 1 : 0);
            bitpos -= 1;
            setBitsFromNumber(reversedBytes, bitpos - 27, bitpos, effectSettings.id);
            bitpos -= 28;
            let parameterIndex = 0;
            for (let p = 0; p < 3 && bitpos - 13 >= 0; p++) {
                let parameter = effectSettings.parameters[parameterIndex++];
                setBitsFromNumber(reversedBytes, bitpos - 12, bitpos, parameter);
                bitpos -= 13;
            }
            ...
            let rightOrderBytes = reversedBytes.reverse();
            offset = result = this.writeSlice(msogDataBuffer, offset, rightOrderBytes);
            success &&= (result !== 0);
            // let reversedBytes = new Uint8Array(effectSectionLength);
            // if (this.msogEffectsReversedBytes[i].length !== reversedBytes.length) {
        ...
        offset = result = this.writeSlice(msogDataBuffer, offset, this.msogUnknown1);
        success &&= (result !== 0);
    ...
    //  111   10     Name
    //  121    1     Unknown byte
    readMSPatch(data, offset) {
        this.MSOG = "MSOG";
        this.msogDataBuffer = data.slice(offset, data.length);
        this.length = data.length - offset;
        this.maxNumEffects = 6; // FIXME add support for other pedals, like MS-60B with 4 effects. See FIXME below on msogNumEffects.
        this.msogEffectsReversedBytes = new Array(this.maxNumEffects);
        this.msogEffectSettings = new Array();
        ...
        for (let i = 0; i < this.maxNumEffects; i++) { // Each effect section is 18 bytes (MSOG_REVERSED_BYTES_SIZE)
            ...
            this.msogEffectsReversedBytes[i] = data.slice(offset, offset + MSOG_REVERSED_BYTES_SIZE).reverse();
            offset += MSOG_REVERSED_BYTES_SIZE;
            let bitpos = this.msogEffectsReversedBytes[i].length * 8 - 1;
            let effectSettings = new EffectSettings();
            effectSettings.enabled = (getNumberFromBits(this.msogEffectsReversedBytes[i], bitpos, bitpos) === 1);
            bitpos -= 1;
            effectSettings.id = getNumberFromBits(this.msogEffectsReversedBytes[i], bitpos - 27, bitpos);
            bitpos -= 28; // One bit less than MS-50G+
            ...
            let parameter = getNumberFromBits(this.msogEffectsReversedBytes[i], bitpos - 12, bitpos);
            bitpos -= 13;
            effectSettings.parameters.push(parameter);
            // P1 = 13 bits
            parameter = getNumberFromBits(this.msogEffectsReversedBytes[i], bitpos - 12, bitpos);
            bitpos -= 13;
            effectSettings.parameters.push(parameter);
            // P2 = 13 bits
            parameter = getNumberFromBits(this.msogEffectsReversedBytes[i], bitpos - 12, bitpos);
            ...
            // P3-P7 = 8 bits
            for (let p = 3; p < 8 && bitpos - 8 >= 0; p++) {
                parameter = getNumberFromBits(this.msogEffectsReversedBytes[i], bitpos - 7, bitpos);
                bitpos -= 8;
                effectSettings.parameters.push(parameter);
            }
            // P8 = 8 bits. It is oddly placed, and we don't know what the surrounding bits are (20 unknown bits before, 8 unknown bits after)
        ...
        this.msogUnknown1 = data.slice(offset, offset + 3);
        offset += 3;
        ...
        this.msogName = this.readString(data, offset, 10);
        offset += 10;
    ...
    static fromPatchData(data, offset = 0) {
    ...
    static createEmptyPTCFPatch(nameLength = 0) {
        let zoomPatch = new ZoomPatch();
        zoomPatch.PTCF = "PTCF";
        zoomPatch.version = 2;
        ...
        let reversedBytes = new Uint8Array(PTCF_EDTB_REVERSED_BYTES_SIZE);
        reversedBytes.fill(0);
        zoomPatch.edtbReversedBytes = new Array();
        zoomPatch.edtbReversedBytes.push(reversedBytes);
        zoomPatch.edtbEffectSettings = new Array();
        let effectSettings = new EffectSettings();
        ...
        zoomPatch.setPrm2BufferFromDerivedValues(zoomPatch.prm2Buffer, true, zoomPatch.prm2InvalidEffectSlot, zoomPatch.prm2PatchVolume, zoomPatch.prm2EditEffectSlot, zoomPatch.edtbEffectSettings.length, zoomPatch.prm2PreampSlot, zoomPatch.prm2BPMSlot, zoomPatch.prm2LineSelSlot, zoomPatch.prm2Tempo, zoomPatch.prm2Routing, zoomPatch.prm2Byte2Lower6Bits, zoomPatch.prm2Byte3Upper4Bits, zoomPatch.prm2Byte9Lower5Bits, zoomPatch.prm2Byte10Bit5, zoomPatch.prm2Byte13, zoomPatch.prm2Byte14, zoomPatch.prm2Byte20Bit1And8, zoomPatch.prm2Byte21Lower4Bits, zoomPatch.prm2Byte22Bits3To7, zoomPatch.prm2Byte23Upper3Bits, zoomPatch.prm2Byte24, zoomPatch.prm2Byte25, zoomPatch.prm2Byte26, zoomPatch.prm2Byte27, zoomPatch.prm2Byte28);
    ...
    static createEmptyMSOGPatch() {
        ...
        zoomPatch.msogEffectSettings = new Array(zoomPatch.maxNumEffects);
        zoomPatch.msogEffectsReversedBytes = new Array(zoomPatch.maxNumEffects);
        for (let i = 0; i < zoomPatch.maxNumEffects; i++) {
            let effectSettings = new EffectSettings(MSOG_NUM_PARAMS_PER_EFFECT);
            effectSettings.enabled = true;
            effectSettings.id = 0;
            effectSettings.parameters = new Array(MSOG_NUM_PARAMS_PER_EFFECT);
    ...
    static noteByteCodeToHtmlSlow(valueString) {
    ...
    static noteByteCodeToHtml(valueString) {
        // https://www.alt-codes.net/music_note_alt_codes.php
        // https://www.fileformat.info/info/unicode/char/1D15E/index.htm
    ...
    static noteHtmlToByteCode(valueString) {
        // 0x17: 16th note
        // 0x18: 8th note
        // 0x19: quarter note
        // 0x1A: half note
    ...
    static noteUTF16ToHtmlSlow(valueString) {
        return valueString.replace(/\uD834\uDD62/g, "&#119138;").replace(/\uD834\uDD61/g, "&#119137;").replace(/\uD834\uDD60/g, "&#119136;").replace(/\uD834\uDD5F/g, "&#119135;").replace(/\uD834\uDD5E/g, "&#119134;");
    }
    static noteUTF16ToHtml(valueString) {
    ...
    static noteHtmlToUTF16(valueString) {
    ...
    static noteUTF16ToByteCode(valueString) {
        return (valueString.length <= 1 || valueString[0] !== "\uD834") ? valueString :
    ...
    static isNoteHtml(valueString) {
        return valueString.length >= 9 && valueString.startsWith('&');
    }
}
//# sourceMappingURL=ZoomPatch.js.map